...
def Hogtimus_Prime(curr_score):
    if is_prime(curr_score):
        return next_prime(curr_score)
    else:
        return curr_score
...
def take_turn(num_rolls, opponent_score, dice=six_sided):
    ...
    # BEGIN PROBLEM 2
    "*** REPLACE THIS LINE ***"
    curr_limit = 25 - num_rolls

    if  num_rolls == 0:
        curr_score = free_bacon(opponent_score)
    else:
        curr_score = roll_dice(num_rolls, dice)
    curr_score = Hogtimus_Prime(curr_score)
    if curr_score > curr_limit:
        curr_score = curr_limit
...
def select_dice(score, opponent_score, dice_swapped):
    ...
    if dice_swapped:
        ...
        dice = six_sided
    # END PROBLEM 4
    if (int(score) + opponent_score) % 7 == 0:
        dice = reroll(dice)
...
#print(take_turn(always_roll(5)(91,10),10,make_test_dice(3)))
...
def run(*args):
    ...
    if args.run_experiments:
        run_experiments()
#reroll = lambda d: make_test_dice(5)
#six_sided = make_test_dice(3)
#s0,s1 = play(always_roll(5), always_roll(3), score0=91, score1=10)
#print(s0,s1)